...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-sparks.ico"
...
Section "Electrum-Sparks" SectionDE
  SetOutPath $INSTDIR
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\sparks"
  ...
  File /r "dist\electrum-sparks\*.*"
  File "icons\electrum-sparks.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-sparks-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-sparks-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-sparks-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-sparks-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-sparks-${PRODUCT_VERSION}.exe" 0


  ;Links sparks: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\sparks" "" "URL:sparks Protocol"
  WriteRegStr HKCU "Software\Classes\sparks" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\sparks" "DefaultIcon" "$\"$INSTDIR\electrum-sparks.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\sparks\shell\open\command" "" "$\"$INSTDIR\electrum-sparks-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-sparks.ico"
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "Electrum-Sparks Wallet"
