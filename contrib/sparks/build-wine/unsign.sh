...
version=`python3 -c "import electrum_sparks; print(electrum_sparks.version.ELECTRUM_VERSION)"`
...
for mine in $(ls dist/*.exe); do
    echo "---------------"
    f=$(basename $mine)
    echo "Downloading https://download.sparkspay.io/$version/$f"
    wget -q https://download.electrum_sparks.org/$version/$f -O signed/$f
