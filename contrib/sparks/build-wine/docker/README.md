...
2. Build image
    ...
    ```
    $ sudo docker build --no-cache -t electrum_sparks-wine-builder-img contrib/build-wine/docker
    ```
...
3. Build Windows binaries
    ...
    $ git checkout $REV
    $ sudo docker run \
        --name electrum_sparks-wine-builder-cont \
        -v $PWD:/opt/wine64/drive_c/electrum_sparks \
        --rm \
        --workdir /opt/wine64/drive_c/electrum_sparks/contrib/build-wine \
        electrum_sparks-wine-builder-img \
        ./build.sh
    ```
...
|-----------------------------------------------------------------------------------------------|
| Upload signatures to 'electrum_sparks-signatures' repo, as `$version/$filename.$builder.asc`         |
