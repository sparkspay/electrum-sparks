...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/Electrum-Sparks-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-sparks\icons\electrum-sparks.ico"
...
Section
  ...
  File /r "dist\electrum_sparks\*.*"
  File "c:\electrum-sparks\icons\electrum-sparks.ico"
  ...
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum-sparks.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum-sparks-${PRODUCT_VERSION}.exe$\" $\"%1$\""
