...
_ = lambda x:x
#from i18n import _
from electrum_sparks import WalletStorage, Wallet
from electrum_sparks.util import format_satoshis, set_verbosity
from electrum_sparks.bitcoin import is_address, COIN, TYPE_ADDRESS
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists:
            print("Wallet not found. try 'electrum-sparks create'")
            exit()
    ...
    def do_send(self):
        if not is_address(self.str_recipient):
            print(_('Invalid Sparks address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-sparks setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-sparks setconfig' to change your settings")
        return True
