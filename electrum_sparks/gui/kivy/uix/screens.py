...
from electrum_sparks.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_sparks import bitcoin
from electrum_sparks .transaction import TxOutput
from electrum_sparks.util import timestamp_to_datetime
...
from electrum_sparks.gui.kivy.i18n import _
...
class CScreen(Factory.Screen):
    ...
    def load_screen(self):
        self.screen = Builder.load_file('electrum_sparks/gui/kivy/uix/ui_screens/' + self.kvname + '.kv')
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_hash, tx_mined_status, value, balance):
        ...
        icon = "atlas://electrum_sparks/gui/kivy/theming/light/" + TX_ICONS[status]
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        if not self.app.wallet:
            ...
            return
        import electrum_sparks
        try:
            uri = electrum_sparks.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a Sparks URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_sparks.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Sparks address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Sparks Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_sparks.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Sparks Request"))
