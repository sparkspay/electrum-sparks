...
try:
    from electrum_sparks.crypto import Hash, EncodeAES, DecodeAES
    from electrum_sparks.bitcoin import (TYPE_ADDRESS, push_script, var_int, public_key_to_p2pkh, is_address,
                                  serialize_xpub, deserialize_xpub)
    from electrum_sparks import ecc
    from electrum_sparks.ecc import msg_magic
    from electrum_sparks.wallet import Standard_Wallet
    ...
    from electrum_sparks.keystore import Hardware_KeyStore
    from ..hw_wallet import HW_PluginBase
    from electrum_sparks.util import print_error, to_string, UserCancelled
    from electrum_sparks.base_wizard import ScriptTypeNotSupported, HWD_SETUP_NEW_WALLET
...
class DigitalBitbox_Client():
    ...
    def dbb_generate_wallet(self):
        ...
        filename = ("Sparks-Electrum-" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".pdf")
        msg = ('{"seed":{"source": "create", "key": "%s", "filename": "%s", "entropy": "%s"}}' % (key, filename, 'Digital Bitbox Sparks-Electrum Plugin')).encode('utf8')
