...
from electrum_sparks.util import PrintError, UserCancelled
from electrum_sparks.keystore import bip39_normalize_passphrase
from electrum_sparks.bitcoin import serialize_xpub, convert_bip32_path_to_list_of_uint32
    ...
    def callback_PassphraseRequest(self, req):
        if self.creating_wallet:
            msg = _("Enter a passphrase to generate this wallet.  Each time "
                    ...
                    "access the Sparks coins in the wallet.").format(self.device)
        else:
...
class KeepKeyClientBase(GuiMixin, PrintError):
    ...
    def has_usable_connection_with_device(self):
        try:
            res = self.ping("electrum-sparks pinging device")
            assert res == "electrum-sparks pinging device"
